...
const generateId = createIdGenerator(todos.length + 1);

const fastify = Fastify({
  logger: true,
});

fastify.register(cors, { methods: "*" });
...
fastify.post("/todos/add", async (request, reply) => {
  const { todo, completed, userId } = request.body;
  const newTodo = { id: generateId(), todo, completed, userId };
  todos.push(newTodo);
  reply.status(201);
  return newTodo;
});
...
fastify.patch("/todos/:id", async (request, reply) => {
  const id = Number(request.params.id);
  const { todo: title, completed } = request.body;
  const todo = todos.find((todo) => todo.id === id);
  if (!todo) {
    reply.status(404);
    return { message: `Todo with id=${id} not found` };
...
function createIdGenerator(initialId = 1) {
  let id = initialId;
  return () => id++;
}
